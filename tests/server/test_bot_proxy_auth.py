...
@pytest.mark.asyncio
async def test_chat_proxy_forwards_runtime_llm_payload(monkeypatch):
    forwarded = {}

    class FakeResponse:
        def __init__(self):
            self.status_code = 200
            self.text = '{"message": "ok", "session_id": "session-1"}'

        def raise_for_status(self):
            return None

        def json(self):
            return {"message": "ok", "session_id": "session-1"}
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.post("/bot/v1/chat", json=payload)

    assert response.status_code == 200
    assert response.json()["message"] == "ok"
    assert forwarded["url"] == "http://127.0.0.1:18790/bot/v1/chat"
    assert forwarded["json"]["runtime_llm"] == payload["runtime_llm"]
    assert forwarded["headers"]["X-Gateway-Token"] == "gateway-secret"
